...
class Game:
    ...
    def __init__(self, controls=(None, None), num_players=1):
        self.bats = [Bat(0, controls[1]), Bat(1, controls[0])]
        self.ball = Ball(-1)
    ...
    def draw(self):
        ...
        imagenn = "help"+str(self.players)
        screen.blit(imagenn, (0,0))
        for p in (0,1):
...
def update():
    ...
    if state == State.MENU:
        if space_pressed:
            ...
            game = Game(controls,num_players)
        else:
